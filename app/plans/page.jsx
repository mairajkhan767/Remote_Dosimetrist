...
export default async function PlansPage() {
  ...
  return (
    <section className="ip-page-enter ip-grain">
      ...
      {
      data?.acf?.treatments_section.map((plan, index) => {
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16 items-center">
                      <div className={isEven ? 'lg:order-1' : 'lg:order-2'}>
                        <div className="ip-image-wrap relative">
                          <img src={plan.image.url} alt={plan.image.alt} className="w-full object-cover object-center h-[260px] md:h-[480px]" />
                      ...
                      <div className={`relative ${isEven ? 'lg:order-2' : 'lg:order-1'}`}>
                        <div className="relative flex flex-col gap-6">
                          ...
                          <h3 className="text-[#003777] text-2xl md:text-[38px] font-extrabold -tracking-[0.95px] leading-[1.1]">{plan.heading}</h3>
                          <p className="text-[#434961] text-[16px] md:text-[18px] leading-[28px] md:leading-[32px] -tracking-[0.2px] max-w-[560px]">{plan.description}</p>
                        </div>
                      </div>
                    </div>
                  </div>
